...
class PatchEmbed3D(nn.Module):
    def __init__(self, voxel_size=(64,64,48), patch_size=4, in_chans=3, embed_dim=384, bias=True):
        super().__init__()
        self.voxel_size = voxel_size
        self.patch_size = patch_size
        self.patch_dims = (
            voxel_size[0] // patch_size,
            voxel_size[1] // patch_size,
            voxel_size[2] // patch_size,
        )
...
class DiT(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_dims = self.x_embedder.patch_dims
    ...
    def initialize_weights(self):
        ...
        if isinstance(self.input_size, int):
            ...
            grid_size = tuple(i // self.patch_size for i in self.input_size)
    ...
    def unpatchify_voxels(self, x):
        B = x.shape[0]
        Dp, Hp, Wp = self.patch_dims
        p = self.patch_size
        c = self.out_channels

        x = x.view(B, Dp, Hp, Wp, p, p, p, c)
        x = x.permute(0, 7, 1, 4, 2, 5, 3, 6).contiguous()
        x = x.view(B, c, Dp*p, Hp*p, Wp*p)
        return x


    def forward(self, x, t, y=None):
        B, C, D, H, W = x.shape  # should be (B, 3, 64, 64, 48)
...
def get_3d_sincos_pos_embed(embed_dim, grid_size, cls_token=False, extra_tokens=0):
    ...
    grid_x = np.arange(gx, dtype=np.float32)
    grid_y = np.arange(gy, dtype=np.float32)
    grid_z = np.arange(gz, dtype=np.float32)

    grid = np.meshgrid(grid_x, grid_y, grid_z, indexing='ij')  # ij for x/y/z order
